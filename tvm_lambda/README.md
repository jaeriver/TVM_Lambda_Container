...
```bash
tvm_lambda$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
tvm_lambda$ sam local start-api
tvm_lambda$ curl http://localhost:3000/classify_digit
```
...
```bash
tvm_lambda$ sam logs -n InferenceFunction --stack-name tvm_lambda --tail
```
...
```bash
aws cloudformation delete-stack --stack-name tvm_lambda
```
